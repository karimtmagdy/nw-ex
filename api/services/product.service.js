...
const { MSG_PRODUCT } = require("../lib/status/msg-status");
...
exports.deleteProduct = fn(async (req, res, next) => {
  ...
  if (!product) return next(new AppError(MSG_PRODUCT.product_not, 404));
  res
    .status(200)
    .json({ status: "success", message: MSG_PRODUCT.product_removed });
});
...
exports.updateProduct = fn(async (req, res, next) => {
  ...
  if (!product)
    return res.status(404).json({ message: MSG_PRODUCT.product_not });
  return res
    .status(200)
    .json({ status: "success", message: MSG_PRODUCT.product_updated, product });
});
