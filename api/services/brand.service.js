...
const { MSG_BRAND } = require("../lib/status/msg-status");

exports.createBrand = fn(async (req, res, next) => {
  const { name, isActive } = req.body;
  const existingBrand = await Brand.findOne({ name });
  if (existingBrand) return next(new AppError(MSG_BRAND.brand_exist, 400));
  const brand = await Brand.create({ name, isActive });
  if (isActive == false) brand.status = "inactive";
  await brand.save();
  res.status(201).json({
    status: "success",
    message: MSG_BRAND.brand_created,
    brand,
  });
});
...
exports.updateBrand = fn(async (req, res, next) => {
  ...
  if (!brand) return next(new AppError(MSG_BRAND.brand_not, 404));
  await brand.save();
  res.status(200).json({
    status: "success",
    message: MSG_BRAND.brand_updated,
    brand,
  });
});
...
exports.deleteBrand = fn(async (req, res, next) => {
  ...
  res.status(200).json({
    status: "success",
    message: MSG_BRAND.brand_removed,
  });
});
